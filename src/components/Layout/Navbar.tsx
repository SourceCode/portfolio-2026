          ...
          <ul className="hidden md:flex items-center space-x-8 list-none m-0 p-0">
            {navLinks.map((link) => (
              <li key={link.name}>
                <Link
                  to={link.path}
                  className={`text-sm font-medium transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-mint rounded px-2 py-1 ${isActive(link.path)
              ...
              {navLinks.map((link) => (
                <li key={link.name}>
                  <Link
                    ...
                    className={`text-lg font-medium block p-2 rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-mint ${isActive(link.path) ? 'text-brand-mint' : 'text-white/70'
                      }`}
                    aria-current={isActive(link.path) ? 'page' : undefined}
                  >
                    {link.name}
