...
const ReflectiveSphereRings: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // Safety: Enforce absolute positioning to prevent stacking if duplicates occur
        renderer.domElement.style.position = 'absolute';
        renderer.domElement.style.top = '0';
        renderer.domElement.style.left = '0';
        renderer.domElement.style.width = '100%';
        renderer.domElement.style.height = '100%';
        renderer.domElement.style.pointerEvents = 'none'; // Allow clicks to pass through if needed

        // Clear any existing children to prevent duplication on HMR/Re-mount
        ...
        ringConfig.forEach((config, i) => {
            ...
            earthGroup.add(group); // Parent to earthGroup
            ...
            laser.visible = false;
            earthGroup.add(laser); // Parent to earthGroup
            ...
            ringsData.forEach(r => {
                ...
                if (r.state === 'IDLE') {
                    if (Math.random() < 0.003) {
                        ...
                        // GET DRONE POSITION IN EARTH LOCAL SPACE
                        const droneWorldPos = new THREE.Vector3();
                        r.drone.getWorldPosition(droneWorldPos);
                        earthGroup.worldToLocal(droneWorldPos); // Convert world pos to earth local pos

                        // TARGET IS ALREADY LOCAL
                        // Raycast / Direction from Center Logic:
                        // Since Drone is orbiting center, direction to center is just -position normalized.
                        // But we want a random point on surface? No, original logic was:
                        // localDir = droneWorldPos (relative to earth center) .normalize()
                        // So target is point on surface directly under drone.

                        const localDir = droneWorldPos.clone().normalize();
                    ...
                    // UPDATE LASER GEOMETRY (IN EARTH LOCAL SPACE)
                    const droneWorldPos = new THREE.Vector3();
                    r.drone.getWorldPosition(droneWorldPos);
                    earthGroup.worldToLocal(droneWorldPos);

                    const targetLocalPos = r.targetLocal.clone().multiplyScalar(0.85); // 0.85 is sphere radius

                    const positions = new Float32Array([
                        droneWorldPos.x, droneWorldPos.y, droneWorldPos.z,
                        targetLocalPos.x, targetLocalPos.y, targetLocalPos.z
                    ]);
